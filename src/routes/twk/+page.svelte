...
<div class="dark:bg-slate-900">
    ...
    <main class="py-0 min-h-screen w-full px-2" >
        <div class="flex justify-between flex-nowrap">
            <div class="mb-10 p-4 text-start">
                <a href="/"><h1 class="font-bold text-xl">SoalCPNS.vercel.app</h1></a>
                <h2 >Kumpulan Latihan Soal CPNS HOTS terbaru</h2>
            </div>
            <ThemeSwitch />
        </div>

        <h3 class="font-bold mb-4 border-b m-4 inline-block">Contoh soal TWK</h3>

        <div class="px-4 mx-auto justify-center md:w-1/2 w-full">
            <!-- {#if !completed} -->
            <!--     <div class="bg-red"> -->
            ...
            {#if items.length > 0}
                ...
                {:else}
                    ...
                    {#if isCorrect}
                        <p class="my-4 font-bold">Benar!</p>
                        <button class="bg-yellow-200 p-4 text-black" on:click={showExplanation}>Pembahasan</button>
                        {#if showHint}
                    ...
                    <button class="bg-blue-200 p-4 mb-10 text-black" on:click={nextQuestion}>Next</button>
                {/if}
            {:else}
